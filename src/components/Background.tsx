...
const DotGrid: React.FC<DotGridProps> = ({
  ...
  const buildGrid = useCallback(() => {
    ...
    if (width === 0 || height === 0) return; // safety
    ...
    for (let y = 0; y < rows; y++)
      for (let x = 0; x < cols; x++)
        newDots.push({ cx: startX + x * cell, cy: startY + y * cell, xOffset: 0, yOffset: 0, _inertiaApplied: false });
  ...
  useEffect(() => {
    ...
    const wrap = wrapperRef.current;
    if (!wrap) return;
    const obs = new ResizeObserver(buildGrid);
    obs.observe(wrap);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (const dot of dotsRef.current) {
        ...
        let col = `rgb(${baseRgb.r},${baseRgb.g},${baseRgb.b})`;
        if (dsq < proxSq) {
    ...
    draw();
  }, [baseRgb, activeRgb, circlePath, proximity, resistance]);

  // FIXED: store callbacks so cleanup removes same reference ✅
  const moveCb = useMemo(
    () =>
      throttle((e: MouseEvent) => {
        ...
        pr.x = Math.floor(pr.x);
        pr.y = Math.floor(pr.y);
        pr.x = pr.x;
        pr.y = pr.y;
    ...
    [dotSize, gap, proximity, resistance]
  );

  const clickCb = useMemo(
    () => (e: MouseEvent) => {
      ...
      for (const dot of dotsRef.current) {
        ...
        if (dist < shockRadius && !dot._inertiaApplied) {
          ...
          gsap.to(dot, {
            inertia: { xOffset: (dot.cx - cx) * shockStrength * f, yOffset: (dot.cy - cy) * shockStrength * f, resistance },
            onComplete: () => {
              gsap.to(dot, { xOffset: 0, yOffset: 0, duration: returnDuration, ease: 'elastic.out(1,0.75)' });
              dot._inertiaApplied = false;
            }
